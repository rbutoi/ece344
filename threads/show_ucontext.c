...
main(int argc, char **argv)
{
	...
	printf("memory address of the variable err = %p\n",
	       (void *) &err);
	long num_between;
	if (&setcontext_called > &err) {
		num_between = &setcontext_called - &err;
	} else {
		num_between = &err - &setcontext_called;
	}
	...
	if ((void *) mycontext.uc_mcontext.gregs[REG_RSP] > (void *) &err) {
		num_between = (void *) mycontext.uc_mcontext.gregs[REG_RSP] - (void *) &err;
	} else {
		num_between = (void *) &err - (void *) mycontext.uc_mcontext.gregs[REG_RSP];
	}
